...
const setSQLParserQuery = (emitenteID, dataEmissao = '', nfe = '', type = 1) => {
    ...
    if (dataEmissao !== '') {
        let dd = new Date(dataEmissao)
        let mindate = new Date("2021-01-01 00:00:00")
        if (dd > mindate) sql += ` AND empresa_notas.nf_emissao LIKE '${dataEmissao}%'`
    }
    ...
    await executeQuery(sql)
        .then(async ([rows, fields]) => {
            ...
            for (const linhas of rows) {
                ...
                let dd = new Date(linhas.nf_emissao)
                let mindate = new Date("2021-01-01 00:00:00")
                if (dd > mindate) {
                    datta.push({
                        destinatario: {
                    ...
                    if (type == 1) return getNfeInfos(res, nfe, data, type = 2)
                }
            }
...
const setNfeDataFromPost = async (req, res) => {
    ...
    let dd = new Date(data.date)
    let mindate = new Date("2021-01-01 00:00:00")
    //Recuda Pesquisa se nf for de data menor que 01/01/2021
    if (dd < mindate) return output(res, 0, "Desculpe, não estão disponíveis as notas fiscais anteriores a 01/01/2021")
